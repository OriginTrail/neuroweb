...
fn public_interfaces_are_ok() {
    ExternalityBuilder::build().execute_with(|| {
        ...
        assert_eq!(
            XcAssetConfig::get_asset_id(asset_location),
            Some(asset_id)
        );
        assert!(XcAssetConfig::get_units_per_second(asset_location).is_none());
        ...
        assert_eq!(
            XcAssetConfig::get_units_per_second(asset_location),
            Some(units)
        );
    })
}
...
fn different_xcm_versions_are_ok() {
    ExternalityBuilder::build().execute_with(|| {
        ...
        assert_ok!(XcAssetConfig::register_asset_location(
            RuntimeOrigin::root(),
            Box::new(VersionedMultiLocation::V2(legacy_asset_location)),
            asset_id
        ));
